...
{
  begin(cols,lines,0,0);
}

void ShiftRegLCD123::begin( uint8_t cols, uint8_t lines, uint8_t font )
{
  begin(cols,lines,font,0);
}

void ShiftRegLCD123::begin( uint8_t cols, uint8_t lines, uint8_t font, uint8_t LCD_CODEPAGE)
{
  _numlines    = lines;
...
#if defined(ARDUINO) && ARDUINO >= 100
size_t ShiftRegLCD123::writecode(uint8_t value)
#else
void ShiftRegLCD123::writecode(uint8_t value) //send char code whisout utf handling
#endif
{
  send(value, HIGH);
#if defined(ARDUINO) && ARDUINO >= 100
...
void ShiftRegLCD123::clear()
{
  ...
  delay(3);    // this command takes a long time!
}
